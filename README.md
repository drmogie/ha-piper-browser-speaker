...
- **Nothing plays, and the browser console shows `Mixed Content` / `NotSupportedError`:** update to `2026.09.15.1` or later — earlier builds could build an `http://` audio link on a dashboard loaded over `https://` (common when Home Assistant's internal URL is a plain-HTTP local address but you're viewing the dashboard through a reverse proxy or remote-access tunnel), which browsers block outright.
- **A speaker's entity shows `unavailable` no matter what, and Settings → Devices & services shows that device with a red "ERROR" badge:** update to `2026.09.16.2` or later and restart Home Assistant — earlier builds had a race that could make a **second** (or later) Piper Browser Speaker device fail to set up at all if it started up at the same moment as another one, which left its entity permanently broken until Home Assistant restarted again on its own.
- **Card shows "Connected" but nothing ever happens:** double check the card's own entity picker actually matches an entity that's `media_player.play_media`-able (Settings → Devices & services → your speaker device). The card's "Connected" status only means it has an open connection for whatever entity ID it's configured with — it doesn't confirm that entity actually exists or works, so a card pointed at the wrong (or broken) entity can look connected while never receiving anything.
